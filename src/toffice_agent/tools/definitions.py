...
def tools_to_system_prompt(tools: list[ToolDefinition] | None = None) -> str:
    ...
    lines.append('<param name="PARAM_NAME"><![CDATA[VALUE]]></param>')
    lines.append("</tool>")
    lines.append("</tool_call>")
    lines.append("")
    lines.append("CRITICAL: Always wrap parameter values in <![CDATA[...]]> blocks.")
    lines.append("This prevents issues with special characters like <, >, & in code.")
    lines.append("")
    lines.append("Example - writing a file with code:")
    lines.append('<tool_call>')
    lines.append('<tool name="write_file">')
    lines.append('<param name="path"><![CDATA[hello.py]]></param>')
    lines.append('<param name="content"><![CDATA[')
